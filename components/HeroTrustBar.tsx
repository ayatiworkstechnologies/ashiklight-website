...
export default function HeroTrustBar() {
  return (
    <div className="bg-[#060E1A] border-y border-[#D4A017]/15 py-3.5 sm:py-5 px-4 sm:px-6 lg:px-12 overflow-hidden">
      <div className="max-w-7xl mx-auto flex items-center justify-start md:justify-center lg:justify-between overflow-x-auto scrollbar-hide flex-nowrap md:flex-wrap gap-6 sm:gap-8 pb-1 md:pb-0">
        {items.map((item, i) => {
          ...
          return (
            ...
            >
              {/* Circle icon */}
              <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-full border border-[#D4A017]/30 bg-[#D4A017]/5 flex items-center justify-center text-[#D4A017] group-hover:bg-[#D4A017] group-hover:text-white transition-all duration-300 shadow-sm shrink-0">
                <Icon className="w-3.5 h-3.5 sm:w-4 sm:h-4" strokeWidth={1.5} />
              </div>
              <span className="text-xs sm:text-sm font-medium text-slate-200 tracking-wide whitespace-nowrap group-hover:text-[#D4A017] transition-colors duration-300">
                {item.label}
              </span>
            </div>
          );
